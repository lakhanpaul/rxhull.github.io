...
const PostImage = styled.div`
  ${(props) =>
    ...
    `}
  ${tw`h-64 bg-center bg-cover rounded-t sm:h-80`}
`
...
const PostContainer = styled.div`
  ...
  ${(props) =>
    ...
    css`
      ...
      ${Post} {
        ${tw`items-center sm:flex-row sm:pr-3`}
      }
      ${PostImage} {
        ${tw`w-full rounded-t sm:h-80 sm:min-h-full sm:w-1/2 sm:rounded-t-none sm:rounded-l`}
      }
      ${PostText} {
        ${tw`pl-8 pr-5`}
      }
      ...
      ${PostDescription} {
        ${tw`mt-4 text-sm font-semibold leading-relaxed text-gray-600`}
      }
      ${AuthorInfo} {
        ${tw`flex items-center mt-8`}
      }
      ${AuthorName} {
        ${tw`mt-0 text-sm font-bold text-gray-700`}
      }
    `}
`
