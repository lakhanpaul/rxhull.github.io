...
const Image = styled.div((props) => [
  ...
  props.imageContain ? tw`bg-no-repeat bg-contain` : tw`bg-cover`,
  props.imageShadow ? tw`shadow` : tw`shadow-none`,
  tw`hidden bg-center rounded lg:block h-144`,
])
...
const QuestionToggleIcon = styled.span`
  ${tw`p-1 ml-2 text-gray-100 transition duration-300 rounded-full bg-primary-500 group-hover:bg-primary-700 group-hover:text-gray-200`}
  svg {
    ${tw`w-4 h-4`}
  }
`
