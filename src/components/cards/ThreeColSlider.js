...
import PriceIcon from 'feather-icons/dist/icons/dollar-sign.svg'
...
const ControlButton = styled(PrimaryButtonBase)`
  ${tw`p-2 mt-4 ml-6 rounded-full sm:mt-0 first:ml-0`}
  svg {
    ${tw`w-6 h-6`}
  }
`
...
const CardImage = styled.div((props) => [
  ...
  tw`w-full h-56 bg-center bg-cover rounded sm:h-64 sm:rounded-none sm:rounded-tl-4xl`,
])
...
const RatingsInfo = styled.div`
  ${tw`flex items-center mt-2 sm:ml-4 sm:mt-0`}
  svg {
...
export default ({ cards = null, heading = 'Popular hotels' }) => {
        ...
        <CardSlider ref={setSliderRef} {...sliderSettings}>
          {cards.map((card, index) => (
            <Card key={index}>
              ...
              <PrimaryButton>Take a look</PrimaryButton>
            </Card>
          ))}
        </CardSlider>
      </Content>
